...
class FluxdayDemoWebpageTest(BaseTestCase):
    ...
    def test_active_demo(self):
        ...
        """
        SOMETIMES IT DETECTS THAT THE 'DEMO' BTN IS ACTIVE AND SOMETIMES IT DOES NOT ! ! !
        """
        active_demo = self.driver.find_element_by_xpath(page_selectors.xp_expr_active_demo)
        self.assertEqual(active_demo.get_attribute('class'), 'active', 'Button DEMO is not active')
